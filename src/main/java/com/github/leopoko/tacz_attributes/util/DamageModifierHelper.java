...
public final class DamageModifierHelper {
    ...
    public static double calculateDamageModifier(LivingEntity shooter, @Nullable ResourceLocation gunId) {
        ...
        // 全体ダメージ倍率
        double globalModifier = getAttributeValue(shooter, CustomAttributes.GUN_DAMAGE.get());

        // 銃種別ダメージ倍率
        double typeModifier = 1.0;
        if (gunType != null) {
            typeModifier = getAttributeValue(shooter, gunType.getDamageAttribute().get());
        }
        ...
        if (isAds) {
            adsHipGlobal = getAttributeValue(shooter, CustomAttributes.ADS_DAMAGE.get());
            if (gunType != null) {
                adsHipType = getAttributeValue(shooter, gunType.getAdsDamageAttribute().get());
            }
        } else {
            adsHipGlobal = getAttributeValue(shooter, CustomAttributes.HIP_FIRE_DAMAGE.get());
            if (gunType != null) {
                adsHipType = getAttributeValue(shooter, gunType.getHipFireDamageAttribute().get());
            }
        }
